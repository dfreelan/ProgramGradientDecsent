...
public class MathProgram {
    ...
    static float[][] getTrainingData(int number) {
        ...
        for (int i = 0; i < data.length; i++) {
            data[i][0] = 1-(2*generator.nextFloat());
            data[i][1] = data[i][0]*data[i][0]*data[i][0]*data[i][0] + data[i][0]*data[i][0]*data[i][0] + data[i][0]*data[i][0] + data[i][0];//(float) (float) Math.sin(Math.sin(data[i][0] * data[i][0]));
        }
        return data;
    }
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < 50000; i++) {
            ...
            if(newErr>totalSquareError || Float.isNaN(newErr)){
                ...
                if(Program.alpha < .000001f){
                    Program.alpha = .000001f;
